...
namespace Hrms.AdminApi.Controllers
{
    ...
    public class ManpowerRequisitionsController : Controller
    {
        ...
        [CustomAuthorize("list-manpower-requisition")]
        [HttpGet]
            ...
            string status, string jobTitle, string startingDate, int? departmentId, int? designationId, int? branchId, int? reportingToEmpId)
        {
            ...
            return Ok(new
            {
                Data = data.Select(x => new
                {
                    ...
                    RequestedByEmpName = x.RequestedByEmp != null 
                                            ? Helper.FullName(x.RequestedByEmp?.FirstName, x.RequestedByEmp?.MiddleName, x.RequestedByEmp?.LastName) 
                                            : x.ProcessedByUser?.UserName,
                    RequestedByEmpCode = x.RequestedByEmp?.CardId,
                    ProcessedByEmpId = x.ProcessedByUser?.EmpId,
                    ProcessedByEmpName = x.ProcessedByUser?.Emp != null 
                                            ? Helper.FullName(x.ProcessedByUser?.Emp?.FirstName, x.ProcessedByUser?.Emp?.MiddleName, x.ProcessedByUser?.Emp?.LastName) 
                                            : x.ProcessedByUser?.UserName,
        ...
        [CustomAuthorize("view-manpower-requisition")]
        [HttpGet("{id}")]
        public async Task<IActionResult> Get(int id)
        {
            ...
            return Ok(new
            {
                City = new
                {
                    ...
                    RequestedByEmpName = data.RequestedByEmp != null 
                                            ? Helper.FullName(data.RequestedByEmp?.FirstName, data.RequestedByEmp?.MiddleName, data.RequestedByEmp?.LastName) 
                                            : data.ProcessedByUser?.UserName,
                    RequestedByEmpCode = data.RequestedByEmp?.CardId,
                    ProcessedByEmpId = data.ProcessedByUser?.EmpId,
                    ProcessedByEmpName = data.ProcessedByUser?.Emp != null 
                                            ? Helper.FullName(data.ProcessedByUser?.Emp?.FirstName, data.ProcessedByUser?.Emp?.MiddleName, data.ProcessedByUser?.Emp?.LastName) 
                                            : data.ProcessedByUser?.UserName,
